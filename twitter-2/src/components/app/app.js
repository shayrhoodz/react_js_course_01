...
export default class App extends Component {
  ...
  toggleProp(label, id) {
    ...
    this.setState(({data}) => {
      ...
      const old = data[index];
      const newItem = {...old, [label]: !old[label]}
  ...
  SearchPost(items, term) {
    ...
    return items.filter( (item) => {
      return item.label.indexOf(term) > -1 // если не чего не найдено, будет возвращено -1
    })
  }
  ...
  render() {
    ...
    return (
      <AppBlock>
        ...
        <PostList 
          posts={visiblePost} // обработчики событий = функция
